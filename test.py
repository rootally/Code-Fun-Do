...
with open('/home/ally/IENC/model/Trained_Graphs/merged_frozen_graph_1.pb', 'rb') as f:
	fileContent = f.read()
...
wtoidx = np.load("/home/ally/IENC/wordmap.npy").tolist()
idxtow = dict(zip(wtoidx.values(), wtoidx.keys()))

# np.save("Dataset/wordmap", wtoidx)
# np.save("Dataset/vocab", vocab)

with open('/home/ally/IENC/model/Decoder/DecoderOutputs.txt', 'r') as fr:
	outputs = fr.read()
